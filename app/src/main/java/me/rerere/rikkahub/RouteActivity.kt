...
import me.rerere.rikkahub.ui.pages.browser.BrowserPage
import me.rerere.rikkahub.ui.pages.cluster.ClusterPage
import me.rerere.rikkahub.ui.pages.plugin.PluginPage
import me.rerere.rikkahub.ui.pages.server.ServerDetailPage
import me.rerere.rikkahub.ui.pages.server.ServerListPage
                ...
                Box(
                    ...
                    NavDisplay(
                        ...
                        entryProvider = entryProvider {
                            ...
                            entry<Screen.Browser> {
                                BrowserPage()
                            }

                            entry<Screen.BrowserWithUrl> { key ->
                                BrowserPage(initialUrl = key.url)
                            }

                            entry<Screen.ServerList> {
                                ServerListPage()
                            }

                            entry<Screen.ServerDetail> { key ->
                                ServerDetailPage(serverId = key.id)
                            }

                            entry<Screen.Cluster> {
                                ClusterPage()
                            }
...
sealed interface Screen : NavKey {
    ...
    @Serializable
    data class BrowserWithUrl(val url: String = "") : Screen

    @Serializable
    data object ServerList : Screen

    @Serializable
    data class ServerDetail(val id: Int) : Screen

    @Serializable
    data object Cluster : Screen

    @Serializable
    data object Workflow : Screen

    @Serializable
    data object Plugin : Screen
}
