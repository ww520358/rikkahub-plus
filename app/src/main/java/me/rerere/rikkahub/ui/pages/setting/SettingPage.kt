        ...
        LazyColumn(
            ...
            item("newFeatures") {
                CardGroup(
                    ...
                    item(
                        onClick = { navController.navigate(Screen.Browser) },
                        leadingContent = { Icon(HugeIcons.GlobalSearch, null) },
                        supportingContent = { Text("内置浏览器、书签管理与浏览历史") },
                        headlineContent = { Text("内置浏览器") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.ServerList) },
                        leadingContent = { Icon(HugeIcons.ServerStack01, null) },
                        supportingContent = { Text("SSH/SFTP 服务器连接与管理") },
                        headlineContent = { Text("服务器连接") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.Cluster) },
                        leadingContent = { Icon(HugeIcons.AiMagic, null) },
                        supportingContent = { Text("多角色 AI 协作：串行/并行/投票") },
                        headlineContent = { Text("AI 助手集群") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.Workflow) },
                        leadingContent = { Icon(HugeIcons.Package, null) },
                        supportingContent = { Text("可视化工作流编排与执行") },
                        headlineContent = { Text("工作流编排") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.Plugin) },
                        leadingContent = { Icon(HugeIcons.Settings03, null) },
                        supportingContent = { Text("插件管理与 APK 动态加载") },
                        headlineContent = { Text("插件系统") },
                    )
                }
            }
