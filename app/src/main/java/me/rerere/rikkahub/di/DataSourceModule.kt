...
val dataSourceModule = module {
    ...
    single {
        get<AppDatabase>().browserHistoryDao()
    }
    ...
    single {
        get<AppDatabase>().pluginDao()
    }

    single {
        me.rerere.rikkahub.ssh.SshManager()
    }

    single {
        me.rerere.rikkahub.cluster.ClusterEngine(
            settingsStore = get(),
            providerManager = get()
        )
    }
    ...
    single {
        me.rerere.rikkahub.workflow.WorkflowEngine(
            database = get(),
            sshManager = get()
        )
    }
    ...
    single {
        me.rerere.rikkahub.plugin.PluginManager(
            context = get(),
            database = get()
        )
    }
